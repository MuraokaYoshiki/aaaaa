...
r = cm[0,1]  #相関係数は，相関行列の0行1列の値

#回帰直線の計算
a1,b1 = np.polyfit( h_wage,p_density,1) 
a2,b2,c2 = np.polyfit( h_wage,p_density,2) 

#回帰直線のための座標計算
x = np.linspace( np.min(h_wage),np.max(h_wage),20)  #ｘ軸は最小値から最大値に20区間
y1 = a1 * x + b1
y2 = a2 * x**2 + b2 * x + c2
...
#回帰直線
plt.plot(x,y1,label=f"y={a1:.3f}x+{b1:.3f}")
plt.plot(x,y2,label=f"y={a2:.3f}x**2+{b2:.3f}*x+{c2:.3f}")
